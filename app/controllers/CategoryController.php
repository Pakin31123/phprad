...
class CategoryController extends BaseController{
	...
	function index($fieldname = null , $fieldvalue = null){
		$db = $this->GetModel();
		$tablename = $this->tablename = 'category';
		$fields = array('id', 'name AS name');
		...
		$this->view->page_title ="Category";
		$this->view->render('category/list.php' , $data ,'main_layout.php');
	}
	/**
     * Load csv|json data
     * @return data
	...
	function import_data(){
		...
		$list_page = (!empty($_POST['redirect']) ? $_POST['redirect'] : 'category/list');
	...
	function view( $rec_id = null , $value = null){
		...
		if(!empty($record)){
			$this->view->page_title ="View  Category";
			$this->view->render('category/view.php' , $record ,'main_layout.php');
		}
		else{
			$page_error = null;
			if($db->getLastError()){
	...
	function add(){
		if(is_post_request()){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename = 'category';
			$fields = $this->fields = array(); //insert fields
		...
		$this->view->page_title ="Add New Category";
		$this->view->render('category/add.php' ,null,'main_layout.php');
	}
	/**
     * Edit Record Action 
	...
	function edit($rec_id = null){
		...
		$this->rec_id = $rec_id;
		$tablename = $this->tablename = 'category';
		$fields = $this->fields = array('id'); //editable fields
		if(is_post_request()){
			Csrf :: cross_check();
			$postdata = $this->transform_request_data($_POST);
			$this->rules_array = array(
		...
		$this->view->page_title ="Edit  Category";
		if(!empty($data)){
			$this->view->render('category/edit.php' , $data, 'main_layout.php');
		}
		else{
			if($db->getLastError()){
